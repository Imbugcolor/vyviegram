...
const postCtrl = {
    ...
    getPostsDiscover: async (req, res) => {
        try {
            ...
            const total = await Posts.find({user: {$nin: newArr}}) 
            ...
            const features = new APIfeatures(Posts.find({user: {$nin: newArr}}), req.query).paginating()
    ...
    savePost: async (req, res) => {
        try {
            const post = await Users.find({_id: req.user._id, saved: req.params.id})
            if(post.length > 0) return res.status(400).json({msg: 'You saved this post.'})

            const save = await Users.findByIdAndUpdate(req.user._id, {
                $push: { saved: req.params.id }
            }, { new: true })

            if(!save) return res.status(400).json({msg: 'This post is not exists.'})
